  ...
  Widget build(BuildContext context) {
    return SafeArea(
      child: Scaffold(
        body: StreamBuilder(
            stream:
                _firestoreController.getInProgressCurrentUserRequestStream(),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                return const CircularLoaderWidget();
              }
              if (snapshot.data!.isNotEmpty) {
                List<RequestModel>? reqList = snapshot.data;
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => RideCompletetionScreen(
                      requestModel: reqList!.first,
                              ...
                              children: [
                                Row(
                                  children: [
                                    CircleAvatar(
                                      radius:
                                          SizeConfig.width10(context) * 8 / 2,
                                      child: widget.driverModel.profilePicture
                                              .isEmpty
                                          ? Image.asset(
                                              Assets.blankProfilePicture,
                                              height:
                                                  SizeConfig.height10(context) *
                                                      10,
                                              width:
                                                  SizeConfig.width10(context) *
                                                      8,
                                            )
                                          : ClipOval(
                                              child: Image.network(
